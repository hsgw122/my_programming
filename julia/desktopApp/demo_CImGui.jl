...
window = GLFW.CreateWindow(1280, 1020, "Demo")
...
try
    ...
    while !GLFW.WindowShouldClose(window)
        ...
        @cstatic f=Cfloat(0.0) counter=Cint(0) begin
            ...
            @c CImGui.Checkbox("Another Window two", &show_another_window_two)
        ...
        # show another simple window two.
        if show_another_window_two
            @c CImGui.Begin("Another Window", &show_another_window_two)  # pass a pointer to our bool variable (the window will have a closing button that will clear the bool when clicked)
            CImGui.Text("Hello from another window!")
            CImGui.Button("Close Me") && (show_another_windowtwo = false;)
            CImGui.End()
        end
